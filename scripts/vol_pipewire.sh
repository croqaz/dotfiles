...
# From: https://github.com/vivien/i3blocks-contrib/tree/master/volume-pipewire
...
DEFAULT_COLOR=${DEFAULT_COLOR:-"#d79921"}
...
function print_block {
    ACTIVE=$(pactl list sinks | grep "State\: \(RUNNING\|IDLE\|SUSPENDED\)" -B4 -A55 | grep "Name:\|Volume: \(front-left\|mono\)\|Mute:\|api.alsa.pcm.card = \|node.nick = ")
...
sed 's/.*= "\(.*\)".*/\1/')
        ...
        DESCRIPTION=$(pactl list sinks | grep "State\: \(RUNNING\|IDLE\)" -B4 -A55 | grep 'Description: ' | sed 's/^.*: //')
